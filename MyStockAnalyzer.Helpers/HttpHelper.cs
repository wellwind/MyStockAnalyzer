...
namespace MyStockAnalyzer.Helpers
{
    public class HttpHelper
    {
        ...
        {
            string randBrowser = getRandomBrowser();

            return randBrowser + " => " + getRandomAgentByBrowser(randBrowser);
        }

        private static string getRandomBrowser()
        {
            List<string> listWeightedBrowser = new List<string>();

            int browserTotalWeight = 0;
            foreach (KeyValuePair<string, int> kvp in _browser_freq)
            {
                browserTotalWeight += kvp.Value;

                for (int i = 0; i < kvp.Value; ++i)
                {
                    listWeightedBrowser.Add(kvp.Key);
                }
            }
            return listWeightedBrowser[rnd.Next(browserTotalWeight)];
        }

        private static string getRandomAgentByBrowser(string browserKey)
        {
            List<string> agentList = _browser_agent[browserKey];
            return agentList.OrderBy(x => rnd.Next()).ToArray()[0];
        }

        private static Dictionary<string, int> _browser_freq = new Dictionary<string, int>()
        {
        ...
        private static Dictionary<string, List<string>> _browser_agent = new Dictionary<string, List<string>>()
        {
            {
                "Internet Explorer",
                new List<string>()
